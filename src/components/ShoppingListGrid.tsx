...
import { SupermarketSector, RecipeWithIngredients } from '../types'
import { SaveIcon, ShareIcon, ArrowLeftIcon, PlusIcon, BookOpenIcon } from './Icons'
...
interface ShoppingListGridProps {
  ...
  onAddRecipes?: (recipeIds: string[]) => Promise<void>
}

export default function ShoppingListGrid({ shoppingList, onBack, onNewList, recipes = [], onAddRecipes }: ShoppingListGridProps) {
  ...
  const [selectedRecipesToAdd, setSelectedRecipesToAdd] = useState<Set<string>>(new Set())
  ...
  const toggleRecipeToAdd = (id: string) => {
    const newSet = new Set(selectedRecipesToAdd)
    if (newSet.has(id)) {
      newSet.delete(id)
    } else {
      newSet.add(id)
    }
    setSelectedRecipesToAdd(newSet)
  }

  const handleAddSelectedRecipes = async () => {
    if (selectedRecipesToAdd.size === 0 || !onAddRecipes) return
    
    setAddingRecipes(true)
    try {
      await onAddRecipes(Array.from(selectedRecipesToAdd))
      setSelectedRecipesToAdd(new Set())
      setShowAddRecipesModal(false)
    } catch (error) {
      console.error('Error adding recipes:', error)
      alert('Failed to add recipes to list')
    } finally {
    ...
    <div>
      ...
      <div className="mb-6">
        ...
        <div className="flex justify-evenly items-center">
          ...
          {recipes.length > 0 && onAddRecipes && (
            <button
              onClick={() => setShowAddRecipesModal(true)}
              className="w-12 h-12 flex items-center justify-center bg-white dark:bg-gray-800 rounded-lg hover:scale-110 transition-transform shadow-md hover:shadow-lg"
              title="Add recipes to list"
            >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl max-w-2xl w-full max-h-[80vh] flex flex-col">
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">Add Recipes to List</h3>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {recipes.map(recipe => {
                  const isSelected = selectedRecipesToAdd.has(recipe.id)
                  return (
                    <button
                      key={recipe.id}
                      onClick={() => toggleRecipeToAdd(recipe.id)}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className={`font-medium ${isSelected ? 'text-primary-700 dark:text-primary-300' : 'text-gray-900 dark:text-white'}`}>
                            {recipe.name}
                          </div>
            ...
            <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex gap-3">
              ...
              <button
                ...
                disabled={selectedRecipesToAdd.size === 0 || addingRecipes}
              ...
              >
                {addingRecipes ? 'Adding...' : `Add ${selectedRecipesToAdd.size} Recipe${selectedRecipesToAdd.size !== 1 ? 's' : ''}`}
              </button>
            </div>
          </div>
        </div>
